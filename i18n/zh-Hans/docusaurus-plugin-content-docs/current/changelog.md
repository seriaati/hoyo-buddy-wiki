...
- 全新的 `/codes` 命令界面，现在可以查看最近的兑换码！
  - (尚未过期的兑换码将以蓝色背景高亮显示)
- 从个人简介中查看或邀请其他用户时的通知将更加明确
- 添加了对绝区零角色的支持
- `/genshin 角色` 现在可以显示圣遗物的主属性和副属性
- `/profile` 的角色展示卡片现在可以使用小号照片
- 更新了原神和崩坏:星穹铁道的新角色数据

### 技术更新

- 重写了部分代码以减少 API 调用次数，获取多个角色的数据时速度更快
- 暂时禁用了部分邮件功能
...
- (`/redeem`) 新增通知设置。
- (`/accounts`) 新增获取 aaid 的说明，方便用户新增米游社账号。
- 新增“隐藏 UI”按钮，适用于按钮数量较多的指令。

### 优化

- (`/search`) 移除“未释出内容”分类，详情请参见[这则消息](https://discord.com/channels/1000727526194298910/1042428379120545873/1346411349999357973)。
...
- 为“缺少权限”和“消息被 AutoMod 阻挡”的错误提供更明确的提示。  
- 简化账号设置过程中的提示文字。  
...
- (`/events`) 修复 HSR 未来卡池未显示为“尚未释出”的问题。  
- 适配新的 ZenlessData 键值。  
...
先前版本中的错误代码导致部分用户在登录时看到“请求次数过多”错误，请参阅[本文](./FAQ.md#尝试使用电子邮件和密码方法新增账户时出现“请求过多”错误) 以了解更多信息。
